...
pub fn simulate_postseason(rng: &mut NbaRng, season: &SeasonData) -> PostseasonResult {
    ...
    let (east_7, east_8) = simulate_play_in(east_seeds[7], east_seeds[8], east_seeds[9], east_seeds[10], rng, season);
    east_seeds[7] = east_7;
    east_seeds[8] = east_8;
    total_games += 3;

    let (west_7, west_8) = simulate_play_in(west_seeds[7], west_seeds[8], west_seeds[9], west_seeds[10], rng, season);
    west_seeds[7] = west_7;
    west_seeds[8] = west_8;
    total_games += 3;
    ...
    let s_e1 = simulate_series(east_seeds[1], east_seeds[8], rng, season);
    total_games += (s_e1.team_a_wins + s_e1.team_b_wins) as u32;
    let east_r1_1 = s_e1.winner;

    let s_e4 = simulate_series(east_seeds[4], east_seeds[5], rng, season);
    total_games += (s_e4.team_a_wins + s_e4.team_b_wins) as u32;
    let east_r1_4 = s_e4.winner;

    let s_e3 = simulate_series(east_seeds[3], east_seeds[6], rng, season);
    total_games += (s_e3.team_a_wins + s_e3.team_b_wins) as u32;
    let east_r1_3 = s_e3.winner;

    let s_e2 = simulate_series(east_seeds[2], east_seeds[7], rng, season);
    total_games += (s_e2.team_a_wins + s_e2.team_b_wins) as u32;
    let east_r1_2 = s_e2.winner;

    let s_w1 = simulate_series(west_seeds[1], west_seeds[8], rng, season);
    total_games += (s_w1.team_a_wins + s_w1.team_b_wins) as u32;
    let west_r1_1 = s_w1.winner;

    let s_w4 = simulate_series(west_seeds[4], west_seeds[5], rng, season);
    total_games += (s_w4.team_a_wins + s_w4.team_b_wins) as u32;
    let west_r1_4 = s_w4.winner;

    let s_w3 = simulate_series(west_seeds[3], west_seeds[6], rng, season);
    total_games += (s_w3.team_a_wins + s_w3.team_b_wins) as u32;
    let west_r1_3 = s_w3.winner;

    let s_w2 = simulate_series(west_seeds[2], west_seeds[7], rng, season);
    total_games += (s_w2.team_a_wins + s_w2.team_b_wins) as u32;
    let west_r1_2 = s_w2.winner;

    // Playoffs Round 2 (Conference Semifinals)
    let s_esf1 = simulate_series(get_higher_seed(east_r1_1, east_r1_4, season), get_lower_seed(east_r1_1, east_r1_4, season), rng, season);
    total_games += (s_esf1.team_a_wins + s_esf1.team_b_wins) as u32;
    let east_sf_1 = s_esf1.winner;

    let s_esf2 = simulate_series(get_higher_seed(east_r1_2, east_r1_3, season), get_lower_seed(east_r1_2, east_r1_3, season), rng, season);
    total_games += (s_esf2.team_a_wins + s_esf2.team_b_wins) as u32;
    let east_sf_2 = s_esf2.winner;

    let s_wsf1 = simulate_series(get_higher_seed(west_r1_1, west_r1_4, season), get_lower_seed(west_r1_1, west_r1_4, season), rng, season);
    total_games += (s_wsf1.team_a_wins + s_wsf1.team_b_wins) as u32;
    let west_sf_1 = s_wsf1.winner;

    let s_wsf2 = simulate_series(get_higher_seed(west_r1_2, west_r1_3, season), get_lower_seed(west_r1_2, west_r1_3, season), rng, season);
    ...
    let s_ecf = simulate_series(get_higher_seed(east_sf_1, east_sf_2, season), get_lower_seed(east_sf_1, east_sf_2, season), rng, season);
    total_games += (s_ecf.team_a_wins + s_ecf.team_b_wins) as u32;
    let east_champ = s_ecf.winner;

    let s_wcf = simulate_series(get_higher_seed(west_sf_1, west_sf_2, season), get_lower_seed(west_sf_1, west_sf_2, season), rng, season);
    ...
    // NBA Finals
    let finals_home = get_higher_seed(east_champ, west_champ, season);
    let finals_away = get_lower_seed(east_champ, west_champ, season);

    let s_finals = simulate_series(finals_home, finals_away, rng, season);
...
fn get_higher_seed(t1: TeamId, t2: TeamId, season: &SeasonData) -> TeamId {
    let team1 = &season.teams[t1.0 as usize];
    let team2 = &season.teams[t2.0 as usize];
    
    if team1.seed > 0 && team2.seed > 0 && team1.seed < team2.seed {
        t1
    } else if team1.seed > 0 && team2.seed > 0 && team2.seed < team1.seed {
        t2
    } else {
        // Tie break by Net Rating
        let idx1 = t1.0 as usize;
        let idx2 = t2.0 as usize;
        let t1_net = season.stats.ortg[idx1] - season.stats.drtg[idx1];
        let t2_net = season.stats.ortg[idx2] - season.stats.drtg[idx2];
        if t1_net >= t2_net { t1 } else { t2 }
    }
}

fn get_lower_seed(t1: TeamId, t2: TeamId, season: &SeasonData) -> TeamId {
    let higher = get_higher_seed(t1, t2, season);
...
mod tests {
    ...
    fn test_simulate_postseason_invariants() {
        ...
        let season = &crate::core::seasons::SEASONS[10];
    ...
    fn test_simulate_postseason_determinism() {
        ...
        let season = &crate::core::seasons::SEASONS[10];
        let res1 = simulate_postseason(&mut rng1, season);

        let mut rng2 = NbaRng::from_seed_and_ids(999, 10, 0);
        let res2 = simulate_postseason(&mut rng2, season);
